...
breweries_by_state %>% ggplot(aes(x=reorder(State, -count), y=count)) +
  geom_col() +
  labs(title = "Breweries by State", y="Number of Breweries", x="")
```

# It would be more interesting to have a map visualization
```{r}
# library(maps)
...
plot_usmap(regions = c("states"), data = breweries_by_state_tomap,
  ...
  label_color = "black") +
  scale_fill_continuous(name = "n*", low="ivory", high="#13294b") +
  theme(legend.position="right", 
        plot.caption = element_text(color="navyblue", size = 8, hjust=1)) +
  labs(title = 'Breweries per Capita', caption = "*breweries per 1,000,000 people")
```
