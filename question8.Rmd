...
synthetic %>% ggplot(aes(x=IBU,y=ABV,color=synth_pred)) + geom_point() + ggtitle(plot_title) +
  scale_color_manual(breaks = c("IPA", "Other"),values=c("#C8102E", "#13294b"))
```
...
Make a simple guess classifier
```{r}
...
for (j in 1:iterations) 
{
  ...
  CM_guess <- confusionMatrix(guess$guess,for_knn$category)
...
The simple guess model has accuracy of 
...
```{r}
beer_nb <- naiveBayes(for_knn[,pred_indices],for_knn$category)
nb_predict <- predict(beer_nb, synth)
synthetic <- cbind(synth,nb_predict)
synthetic %>% ggplot(aes(x=IBU,y=ABV,color=nb_predict)) + geom_point() +
