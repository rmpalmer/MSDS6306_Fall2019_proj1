...
CM_always <- confusionMatrix(always_ipa$guess,infill_normalized$category)
...
for (j in 1:iterations) 
{
  ...
  CM_guess <- confusionMatrix(guess$guess,infill_normalized$category)
...
The random guess model has accuracy of `r format(100*mean_accuracy,digits=2)` %.

Its sensitivity is `r format(100*mean_sensitivity,digits=2)` %.

Its specificity is `r format(100*mean_specificity,digits=2)` %.
...
```{r}
t.test(IBU ~ category,infill_normalized,var.equal=FALSE)
```

```{r}
t.test(ABV ~ category,infill_normalized,var.equal=FALSE)
```
...
accuracy of `r format(100*mean_accuracy,digits=2)` %

sensitivity of `r format(100*mean_sensitivity,digits=2)` %

specificity of `r format(100*mean_specificity,digits=2)` %
...
Do KNN classification.  Make 100 iterations of the model, splitting the data
