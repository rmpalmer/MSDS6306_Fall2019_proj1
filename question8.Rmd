...
for_knn <- only_ales %>% drop_na(ABV,IBU) %>%
...
# how many observations were dropped for missing ABV or IBU
drop_count <- nrow(only_ales) - nrow(for_knn)

# percentages after dropping na
valid_ipa_count   <- for_knn %>% filter(category == 'IPA') %>% nrow()
valid_other_count <- for_knn %>% filter(category == 'Other') %>% nrow()
valid_ipa_percent <- 100 * valid_ipa_count / (valid_other_count + valid_ipa_count)
valid_other_percent <- 100 *valid_other_count / (valid_other_count + valid_ipa_count)
```

Out of a total of `r total_count` beers, `r ipa_count` are IPA and `r other_count` are other ales.

Of these `r ipa_count + other_count` ales, `r drop_count` are excluded from the following analysis
because of missing either ABV or IBU values.

After dropping observations missing either ABV or IBU,
`r valid_ipa_percent`% of ales are IPA and `r valid_other_percent`% are other (not IPA).
...
for_knn %>% ggplot(aes(x=ABV)) +
  geom_histogram(aes(y=..density..,fill=category),show.legend = FALSE,bins=21) +
  facet_grid(category~.) +
