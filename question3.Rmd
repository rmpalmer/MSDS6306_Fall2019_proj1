...
# Find the style of each beer with a missing value, calculate the median ABV for that style of beer, and fill the missing value with the median for that style. Filled IBU values are placed directly back into the "merged" dataset.
...
Fill IBU missing values: ***note that I use "set.seed(8)" to ensure that the model predictions are always the same. My understand is that this consistency should ensure that any downstream dependency shouldn't break. (?)
```{r}
########################################################################
...
merged$IBU[missingIBUindex] <- predict(rtree, forFilling_IBU_rtree)
