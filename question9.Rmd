...
forkmeans <- q9 %>% select(nBreweriesPerSqMile,nStyles, medianabv, medianibu, ia_ratio)
...
# # find optimal "k"
# library(factoextra)
fviz_nbclust(forkmeans, kmeans, method="silhouette")
# 
set.seed(10)
brewsCluster <- kmeans(forkmeans, 3, nstart=20)
str(brewsCluster)
brewsCluster$cluster
cluster_container = c()
for (i in 1:length(brewsCluster$cluster))
...
forkmeans_tomap %>% ggplot(aes(x=nBreweriesPerSqMile, y=nStylesPerCapita, col=cluster, label=row.names(forkmeans_tomap))) +
  geom_text() +
  labs(x='Styles per Capita', y = 'Number of Styles Currently Brewed')

forkmeans_tomap %>% ggplot(aes(x=medianibu, y=ia_ratio, col=cluster, label=row.names(forkmeans_tomap))) +
  geom_text() +
  labs(x='Styles per Capita', y = 'Number of Styles Currently Brewed')
```
