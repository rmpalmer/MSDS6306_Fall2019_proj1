...
abv_df <- merged %>% drop_na(ABV) %>% group_by(State) %>% 
  summarize(median_abv = median(ABV), median_abvfill = median(ABVfill)) 
...
abv_df %>% mutate(compare_abv = median_abv - median_abvfill) %>% 
...
ibu_df <- merged %>% drop_na(IBU) %>% group_by(State) %>% 
  summarize(median_ibu = median(IBU), median_ibufill = median(IBUfill)) 
...
ibu_df %>% mutate(compare_ibu = median_ibu - median_ibufill) %>% summary()
